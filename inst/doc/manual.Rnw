...
\subsection{Global and User Space, Read-Only Tracks}
...
A track may be marked as read-only to prevent its accidential deletion or modification. Use \texttt{emr\_track.readonly} to set or get read-only property of the track. A newly created tracks is always writable. If you wish to mark it as "read-only", please do it in a separate call.
...
Though not officially enforced attributes are intended to store relatively short character strings. Please use \emph{track variables} to store data in any other format. 

A single attribute can be retrieved, added, modified or deleted using \texttt{emr\_track.attr.get} and \texttt{emr\_track.attr.set} functions. Bulk access to more than one attribute is facilitated by \texttt{emr\_track.attr.export} function. \newline

\noindent\fbox{
\parbox{\textwidth}{
\textbf{R}

Track names which attributes values match a pattern can be retrieved using \texttt{emr\_track.ls}, \texttt{emr\_track.global.ls} and \texttt{emr\_track.user.ls} functions.
}
} \newline
...
\begin{tabular}{|l|p{2in}|p{2in}|}
  ...
  Single value retrieval & \texttt{emr\_track.attr.get} & \texttt{emr\_track.var.get} \\
  Bulk value retrieval & \texttt{emr\_track.attr.export} & --- \\
  Single value modification & \texttt{emr\_track.attr.set} & \texttt{emr\_track.var.set} \\
  Object names retrieval & \texttt{emr\_track.attr.export} & \texttt{emr\_track.var.ls} \\
  Object removal & \texttt{emr\_track.attr.rm} & \texttt{emr\_track.var.rm} \\
  Search by value & R: \newline \texttt{emr\_track.ls}, \texttt{emr\_track.global.ls}, \texttt{emr\_track.user.ls} & --- \\
