...
\begin{verbatim}
emr_vtrack.create("annual_glucose", "glucose_track", func="quantile", param=0.5,
                  time.shift=c(-24*365, 0))
\end{verbatim}

This call creates a new virtual track named \texttt{annual\_glucose} based on the underlying physical track \texttt{glucose\_track}. For each iterator point with time \texttt{T} we look at values of \texttt{glucose\_track} in the time window of \texttt{[T-24*365, T]}, i.e. one year prior to \texttt{T}. We calculate then the median over the values (\texttt{func="quantile"}, \texttt{param=0.5}).
...
\begin{verbatim}
emr_extract("annual_glucose")
\end{verbatim}

In addition to altering (expanding or shifting) the time of the iterator point virtual tracks allow also to remap the patient id. This is done via \texttt{id.map} parameter which accepts a data frame that defines the id mapping. Remapping ids might be useful if family ties are explored. For example, instead of glucose level of the patient we are interested to check the glucose level of one of its family members.
...
\emph{Filter} expands the functionality of the iterator by allowing to filter out certain iterator points. Filter is a logical expression consisting of \emph{named} or \emph{unnamed} \emph{elementary filters} (the "building bricks" of the filter) connected with the logical operators: \texttt{\&}, \texttt{|}, \texttt{!} and brackets \texttt{()}.
...
In addition to track names other types of objects can be used within the filter. These are: \emph{Id-Time Points Table}, \emph{Ids Table} and \emph{Time Intervals Table} (see \emph{Appendix} for the format of these tables). When used in the filter the object should be constructed in advanced and be referred by its name. "In place" construction (aka: \texttt{filter = "data.frame(...)"} is not allowed.

\section{Advanced Naryn}

\subsection{Random Algorithms}
Various functions of Naryn such as \texttt{emr\_quantiles} make use of pseudo-random number generator that requires a \emph{seed} - an integer value that determines the particular sequence of the pseudo-random numbers. Thus two identical calls to a function might produce different results if the function uses pseudo-random number generator and the seed has been altered between the calls. Similarly the two calls will generate identical results if the seed did not change between the calls. The value of the seed is controlled by an option \texttt{emr\_rnd.seed}. Any integer number can be used as a seed. The default is \texttt{1}. If \texttt{emr\_rnd.seed} is set to \texttt{0} the actual value of the seed is picked up randomly by the function itself.
...
\begin{tabular}{|l|l|p{3.5in}|}
  ...
  & & \\
  \texttt{emr\_rnd.seed} & \texttt{1} & Seed value for pseudo-random number generator. If '0' seed is set randomally.  \\
\hline
\end{tabular}
