...
{
	m_isend = false;
	m_point.init(g_db->minid(), m_stime, m_keepref ? 0 : NRTimeStamp::NA_REFCOUNT);
...
inline bool NRBeatIterator::next()
{
    ...
    while (++m_point.id <= g_db->maxid()) {
        if (g_db->is_in_subset(m_point.id)) {
            m_point.timestamp.init(m_stime, m_keepref ? 0 : NRTimeStamp::NA_REFCOUNT);
            return true;
        }
    }
...
inline bool NRBeatIterator::next(const NRPoint &jumpto)
{
    unsigned id = jumpto.id;

    if (g_db->is_in_subset(id)) {
        NRTimeStamp::Hour hour = m_period * (unsigned)ceil((jumpto.timestamp.hour() - m_stime) / (double)m_period) + m_stime;
    ...
    while (++id <= g_db->maxid()) {
        if (g_db->is_in_subset(id)) {
            m_point.init(id, m_stime, m_keepref ? 0 : NRTimeStamp::NA_REFCOUNT);
            return true;
        }
    }
