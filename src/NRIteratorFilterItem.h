...
public:
    ...
    bool is_passed(const NRPoint &point) { return is_leaf() ? is_passed_leaf(point) : is_passed_node(point); }

    const NRPoint &jumpto() const { return m_jumpto; }
...
protected:
    ...
    bool is_passed_node(const NRPoint &point);
    bool is_passed_leaf(const NRPoint &point);
    bool filtered_next();
    bool filtered_end();
...
inline bool NRIteratorFilterItem::filtered_next()
{
    ...
    m_jumpto.init(m_itr->point().id, hour, (NRTimeStamp::Refcount)-1);
    return false;
}
