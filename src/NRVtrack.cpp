...
SEXP emr_check_vtrack_attr_src(SEXP _src, SEXP _envir) {
    try {
        ...
        if (isString(_src)) {
            ...
            if (!isVector(_src) || Rf_length(_src) != 2 ||
                !isLogical(VECTOR_ELT(_src, 1)))
...
SEXP emr_check_vtrack_attr_time_shift(SEXP _tshift, SEXP _envir) {
    try {
        ...
        // check the arguments
        if (!(isReal(_tshift) || isInteger(_tshift)) ||
        ...
        if (Rf_length(_tshift) == 1)
            sshift = eshift =
                isReal(_tshift) ? (int)REAL(_tshift)[0] : INTEGER(_tshift)[0];
        else {
            sshift =
                isReal(_tshift) ? (int)REAL(_tshift)[0] : INTEGER(_tshift)[0];
            eshift =
                isReal(_tshift) ? (int)REAL(_tshift)[1] : INTEGER(_tshift)[1];
        }
    ...
    try {
        ...
        if (!isNull(_id_map)) {
            ...
            bool time_shift_used =
                xlength(_id_map) >= NUM_COLS && xlength(colnames) >= NUM_COLS &&
            ...
            for (unsigned i = 0; i < num_ids; ++i) {
                ...
                if (time_shift_used)
                    time_shift = isReal(rtime_shift) ? REAL(rtime_shift)[i]
