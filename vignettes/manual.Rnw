...
\texttt{.ids} file contains available ids that are used to run certain types of \emph{track expression iterators} (see below). The source of these ids comes from a \texttt{dob} (i.e. Date Of Birth) track, which must be present in the global root directory before these iterators may be utilized.
...
\emph{Time intervals iterator} generates points for all the ids that appear in 'dob' track with times taken from a \emph{time intervals table} (see: Appendix). Each time starts at the beginning of the time interval and runs to the end of it with a step of 1. That being said the points that lie outside of \texttt{[stime, etime]} range are skipped.
...
\emph{Beat Iterator} generates a "time beat" at the given period for each id that appear in 'dob' track. The period is given always in hours.
