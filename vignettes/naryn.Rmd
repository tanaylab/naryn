...
If we wish to extract, the same information as before, but in this case we are interested only in patients which have an hemoglobin level of atleast 16 (in addition to our drug treatment requirments). 
...
| Variables | Defined in global environment: <br>`EMR_GROOT`<br>`EMR_UROOT` | Defined in  module's environment:<br>`_GROOT`<br>`_UROOT`
